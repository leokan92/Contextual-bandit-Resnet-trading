    ...
    ##################################################################
        ...
        if i_episode > 3:
            if valid_hist[-1]>max_valid:
                ...
                torch.save(policy_net.state_dict(),os.path.join(settings.SAVE_PARAMS,'policy_DQL_parmeters.pt'))
    ##################################################################
        toc = time.time()
...
def test_dql_run(test_env,M,comission,decay,fold,gamma,n_epoch,asset_name):     
    ...
    total_reward_test = 0
    policy_net = DQN(M).to(device)
    policy_net.load_state_dict(torch.load(os.path.join(settings.SAVE_PARAMS,'policy_DQL_parmeters.pt')))
    step = 0
    done = False
    while not done:      
        a = torch.max(policy_net(state)[0],0)[1].item()
