...
for asset_name, data_name in zip(assets, input_datas):
    ...
    dql = np.sum(np.cumsum(np.load(os.path.join(path_input,'DQN_agent_returns___'+asset_name+'.npy'))))
...
for asset_name, data_name in zip(assets, input_datas):
    ...
    dql = calculate_AR(np.load(os.path.join(path_input,'DQN_agent_returns___'+asset_name+'.npy')),firs_value_test)
