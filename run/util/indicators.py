...
def add_indicators(df):
    df['RSI'] = tam.rsi(df["Close"])
    df['MFI'] = tam.money_flow_index(
        df["High"], df["Low"], df["Close"], df["Volume BTC"])
    df['TSI'] = tam.tsi(df["Close"])
    df['UO'] = tam.uo(df["High"], df["Low"], df["Close"])
    df['AO'] = tam.ao(df["High"], df["Low"])

    df['MACD_diff'] = tat.macd_diff(df["Close"])
    df['Vortex_pos'] = tat.vortex_indicator_pos(
        df["High"], df["Low"], df["Close"])
    df['Vortex_neg'] = tat.vortex_indicator_neg(
        df["High"], df["Low"], df["Close"])
    df['Vortex_diff'] = abs(
        df['Vortex_pos'] -
        df['Vortex_neg'])
